    ...
    <style>
        ...
        * {
        ...
        .main {
            ...
            display: flex;
            flex-direction: column;
            overflow: hidden; /* Fix scroll issues */
            position: relative;
        }
        ...
        /* VIEW CONTAINERS */
        #view-dashboard { display: block; height: calc(100vh - 70px); overflow: hidden; }
        ...
        /* SCROLLBAR STYLING */
        .left-col::-webkit-scrollbar, #view-table::-webkit-scrollbar, .queue-list::-webkit-scrollbar { width: 6px; }
        .left-col::-webkit-scrollbar-track { background: #f1f1f1; }
        .left-col::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 3px; }
        ...
        .radio-group { display: flex; gap: 5px; align-items: center; }
        ...
        .queue-tab { cursor: pointer; transition: all 0.2s; opacity: 0.5; border: 2px solid transparent; }
        .queue-tab.active-tab { opacity: 1; transform: scale(1.05); box-shadow: 0 2px 5px rgba(0,0,0,0.1); border-color: rgba(0,0,0,0.1); }
...
<div id="app">
    ...
    <!-- MODAL (Placed correctly inside App) -->
        ...
        <nav>
            ...
            <div id="managedTicketsWrapper">
                ...
                <div class="dropdown-menu" id="ticketDropdown">
                    <a onclick="loadTable('completed')" class="nav-sub-item"><i class="fas fa-check-circle"></i> Completed</a>
                    <a onclick="loadTable('pending')" class="nav-sub-item"><i class="fas fa-clock"></i> Pending</a>
                    <a onclick="loadTable('skipped')" class="nav-sub-item"><i class="fas fa-forward"></i> Skipped</a>
                    <a onclick="loadTable('cancelled')" class="nav-sub-item"><i class="fas fa-ban"></i> Cancelled</a>
                </div>
            </div>
                ...
                <div class="left-col">
                    <div class="card">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 20px;">
                            <h2 style="margin:0; color:var(--primary); font-size:1.4rem;">Triage Screening Form</h2>
                            <div style="display:flex; gap: 10px;">
                                <span style="background:#e0f2fe; color:#0284c7; padding:6px 12px; border-radius:20px; font-size:0.85rem; font-weight:bold; display:flex; align-items:center; gap:5px;">
                                    <i class="fas fa-ticket-alt"></i> Ticket: <span id="currentTicket">{{ticketid}}</span>
                                </span>
                        ...
                        <div class="form-grid-3">
                            <div><label class="form-label">First Name</label><input type="text" class="form-input"  name="fname" id="fname" placeholder="Ex: Juan" required v-model="fname"></div>
                            <div><label class="form-label">Middle Initial</label><input type="text" class="form-input"  name="Mname" id="mname" placeholder="Ex: D." required v-model="mname"></div>
                            <div><label class="form-label">Last Name</label><input type="text" class="form-input" id="lname" name="lname" placeholder="Ex: Dela Cruz" required v-model="lname"></div>
                        </div>

                        <div class="form-grid-3">
                            <div><label class="form-label">Date of Birth</label><input type="date" class="form-input" id="dob"  v-model="BOD"></div>
                              <div><label class="form-label">Age</label><input type="text" class="form-input" id="age" name="age" placeholder="Age" required v-model="age"></div>
                            <div><label class="form-label">Sex</label><select class="form-input" v-model="savedData.gender"><option>Male</option><option>Female</option></select></div>
                        </div>

                        <div class="form-grid-2">
                          <div>
  <label class="form-label">Civil Status</label>
  <select class="form-input" v-model="savedData.civil_Status">
    ...
    <option value="Widowed">Widowed</option>
                        ...
                        <div style="margin-bottom:15px;">
                            ...
                            <input type="text" class="form-input" placeholder="House No, Street, Barangay, City/Municipality" v-model="home_address">
                        </div>
  ...
  <span class="checklist-question">1. Fever (Temperature > 38°C)?</span>

  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="sym_fever" value="yes" v-model="savedData.sym_fever"> Yes
    </label>

    <label class="radio-label">
      <input type="radio" name="sym_fever" value="no" v-model="savedData.sym_fever"> No
    </label>
  </div>
</div>

                           <div class="checklist-item">
  <span class="checklist-question">2. Cough and/or Colds?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="sym_cough" value="yes" v-model="savedData.has_cough"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="sym_cough" value="no" v-model="savedData.has_cough"> No
    </label>
  </div>
</div>

                            <div class="checklist-item">
  <span class="checklist-question">3. Sore Throat / Throat Pain?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="sym_throat" value="yes" v-model="savedData.has_sorethroat"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="sym_throat" value="no" v-model="savedData.has_sorethroat"> No
    </label>
  </div>
</div>

                          <div class="checklist-item">
  <span class="checklist-question">4. Shortness of Breath / Difficulty Breathing?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="sym_sob" value="yes" v-model="savedData.has_shortnessBreath"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="sym_sob" value="no" v-model="savedData.has_shortnessBreath"> No
    </label>
  </div>
</div>

                           <div class="checklist-item">
  <span class="checklist-question">5. Influenza-like symptoms (Headache, etc)?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="sym_flu" value="yes" v-model="savedData.has_influenza_Symptoms"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="sym_flu" value="no" v-model="savedData.has_influenza_Symptoms"> No
    </label>
  </div>
</div>
                            ...
                            <div class="checklist-item">
  <span class="checklist-question">1. History of confirmed COVID-19 Infection?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="hist_covid" value="yes" v-model="savedData.has_history_Covid"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="hist_covid" value="no" v-model="savedData.has_history_Covid"> No
    </label>
  </div>
</div>

                           <div class="checklist-item">
  <span class="checklist-question">2. Travel/Residence in area with local transmission?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="hist_localtrans" value="yes" v-model="savedData.have_localTransimission"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="hist_localtrans" value="no" v-model="savedData.have_localTransimission"> No
    </label>
  </div>
</div>

                          <div class="checklist-item">
  <span class="checklist-question">3. Contact with someone with recent travel?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="hist_recentTravelContact" value="yes" v-model="savedData.have_contact_recentTravel"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="hist_recentTravelContact" value="no" v-model="savedData.have_contact_recentTravel"> No
    </label>
  </div>
</div>

                             <div class="checklist-item">
  <span class="checklist-question">4. Are there any influenza-like illness during the week in your workplace?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="hist_influenza" value="yes" v-model="savedData.has_inluenza_illness"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="hist_influenza" value="no" v-model="savedData.has_inluenza_illness"> No
    </label>
  </div>
</div>

                            <div class="checklist-item">
  <span class="checklist-question">5. Did you have direct contact with suspected or confirmed case?</span>
  <div class="radio-group">
    <label class="radio-label">
      <input type="radio" name="hist_confirmed" value="yes" v-model="savedData.has_contactConfirm"> Yes
    </label>
    <label class="radio-label">
      <input type="radio" name="hist_confirmed" value="no" v-model="savedData.has_contactConfirm"> No
    </label>
  </div>
</div>
                            ...
                            <div class="checklist-item" style="flex-direction: column; align-items: flex-start;" >
                                ...
                                <input type="text" class="form-input" placeholder="e.g., Hypertension, Diabetes, Asthma..." v-model="savedData.existingConditions">
                            </div>
                                   <div class="checklist-item" style="flex-direction: column; align-items: flex-start;">
                            <span class="checklist-question" style="margin-bottom: 5px;">2. if yes, please specify date of admission:</span>
                            <input type="text" class="form-input" placeholder="e.g., Hypertension, Diabetes, Asthma..." v-model="savedData.admissionDate">
                        </div>
                        <div class="checklist-item">
                            <span class="checklist-question">3. Were you admitted recently due to these conditions?</span>
                            <div class="radio-group">
                                <label class="radio-label"><input type="radio" name="hist_admit" value="yes" v-model="savedData.admitted_conditions"> Yes</label>
                                <label class="radio-label"><input type="radio" name="hist_admit" value="no" v-model="savedData.admitted_conditions"> No</label>
                            </div>
                        </div>
                        ...
                        <div class="checklist-item">
                            ...
                            <div class="radio-group">
                                <label class="radio-label"><input type="radio" name="hist_icu" value="yes" v-model="savedData.historyICU"> Yes</label>
                                <label class="radio-label"><input type="radio" name="hist_icu" value="no" v-model="savedData.historyICU"> No</label>
                            </div>
                        </div>
                        ...
                        <button class="btn btn-submit" @click="savedsData()">
                ...
                <div class="right-col">
                    <div class="card" style="padding:0; overflow:hidden; height:100%; display:flex; flex-direction:column;">
                       <div style="padding:20px; background:#f8fafc; border-bottom:1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <h3 style="color:var(--text-dark); margin: 0; font-size:1.1rem;">Incoming Patients</h3>
                            ...
                            <div style="display: flex; gap: 8px;">
                                <div id="tab-priority" onclick="switchQueueTab('priority')" class="queue-tab" style="background: #fee2e2; color: #b91c1c; font-weight: bold; padding: 5px 12px; border-radius: 8px; font-size: 0.85rem; display:flex; align-items:center; gap:5px;">
                                    <i class="fas fa-exclamation-circle"></i> {{pendingTickets.length}}
                                </div>
                                ...
                                <div id="tab-pending" onclick="switchQueueTab('pending')" class="queue-tab active-tab" style="background: #e0f2fe; color: #0369a1; font-weight: bold; padding: 5px 12px; border-radius: 8px; font-size: 0.85rem; display:flex; align-items:center; gap:5px;">
                                    <i class="fas fa-user-clock"></i> 12
                                </div>
                            </div> 
                        </div>
     ...
     class="queue-item"
     :class="{ 'active': selectedTicket && selectedTicket.patient_id === ticket.patient_id }" 
...
<div class="queue-footer">
    <div class="queue-btn-grid">
        <!-- 
           Check if selectedTicket exists using optional chaining (?.) 
           or ensure the buttons are disabled if no ticket is selected 
        -->
        
        <button class="q-btn btn-call" 
                @click="callnumber(selectedTicket?.patient_id,selectedTicket?.ticket)"
                >
        ...
        <!-- Next usually doesn't need an ID, but you can pass it if needed -->
        <button class="q-btn btn-next" 
                @click="queueAction('next')">
...
<script>
    ...
    function queueAction(type) {
        ...
        if(type === 'call') alert("Announcing ticket again...");
            ...
            if(type === 'skip' || type === 'cancel') {
            // This function is already defined in your code (Line 414)
    ...
    function submitTriage() {
      ...
      window.currentStation = "<?php echo isset($_SESSION['jobassign']) ? $_SESSION['jobassign'] : 'OPERATING ROOM'; ?>";
</script>
</body>
</html>